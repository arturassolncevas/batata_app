...
const PrivateRoute = ({ component: Component, allowed_roles = [], ...rest }) => {
  let { isLogged, user: { roles = [] } } = store.getState().authReducer
  return(
    <Route
      {...rest}
      render={props => isLogged ? (authorize(roles, allowed_roles) ?
