...
class OrdersPage extends Component {
  ...
  filterOrdersRequest(data) {
    if (this.props.history.location.pathname !== "/orders")
      return
    return requestClient.post(`/api/orders/filter`, { data: { ...data, ...this.state.placed_orders.sort, page: this.state.placed_orders.pagination.page } })
      .then(async (response) => {
        ...
        <Row justify="center">
          <Col xl={24} style={{ textAlign: "center", padding: "40px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <OrdersFilter
                  data={{}}
                  filterCallback={(data) => { handleFilter(data) }}
                />
              <div>
                <Select
...
export default withRouter(injectIntl(OrdersPage))